...
Нативная macOS-заставка с трёхмерной Землёй, текущим освещением день/ночь и аналоговыми часами городов. Рабочее название — **Terra**, репозиторий — **Solstice**. Версия проекта: **1.02**.
...
IP-геолокация доступна по запросу пользователя. Перед первым обращением Preview явно сообщает, что внешний IP будет отправлен сервису ipwho.is, и предлагает разрешить или отклонить запрос. После разрешения камера, защищённый текущий город и ориентация Луны рассчитываются для возвращённого приблизительного города; повторный запрос выполняется не чаще раза в 2 минуты. Заставка использует IP только после сохранённого согласия в Preview. При отказе или ошибке начальный вид — Madrid. VPN влияет на результат. Текстуры и астрономические вычисления работают локально. Нет фоновых служб, расширений входа, launch agents, login items или замен системного экрана авторизации.
...
## 13. Производительность и известные границы версии 1.02
...
- Не проведены длительное измерение энергопотребления через Instruments и проверка реального системного lock screen. Короткий сравнительный замер Preview на текущем Mac показал около 12–14% CPU при 30 fps, около 5–7% при 15 fps и примерно 0,5–2% при 1 fps; штатный режим — 1 fps. Результаты зависят от Mac и дисплея.
...
Город из IP/VPN добавляется в список часов автоматически, его координаты и часовой пояс берутся из ответа. Проверка выполняется при старте и затем не чаще раза в две минуты; ракурс плавно обновляется. При ошибке сохраняется прежний город.
