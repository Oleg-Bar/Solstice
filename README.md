...
Нативная macOS-заставка с трёхмерной Землёй, текущим освещением день/ночь и аналоговыми часами городов. Рабочее название — **Terra**. Версия проекта: **0.3.0**.

![Terra Preview](Docs/Observer-Singapore.png)

Кадр получен настоящим Metal-движком, с фиксированным временем **6 сентября 2026, 12:00 UTC, Singapore** для воспроизводимой проверки. При обычном запуске используется текущее время Mac.
...
Автоматическая IP-геолокация по умолчанию отключена до согласия на сервис ipwho.is. При включении сервис получает внешний IP Mac, а камера поворачивается к возвращённому приблизительному городу; запрос при старте и не чаще раза в 2 минуты. При ошибке сохраняется текущий вид, начальный — Madrid. В preview доступны ручные Madrid, Singapore и New York. VPN влияет на результат. Текстуры и астрономические вычисления работают локально. Нет фоновых служб, расширений входа, launch agents, login items или замен системного экрана авторизации.
...
- Параметры — **Command–,** или меню **Вид → Настроить сцену…**.
- Завершить — **Command–Q**.
...
Скрипт собирает код для архитектуры текущего Mac, копирует локальные ресурсы и выполняет ad-hoc подпись обоих bundle, затем проверяет подпись. На этой машине результат — arm64. Для M1 этого достаточно. Это не универсальный дистрибутив для всех Mac.
...
Панель preview меняет город наблюдателя, размер/высоту Земли, вращение, видимость Луны и режим масштаба расстояния, яркость звёзд и Млечного Пути, атмосферу, размеры часов и подписей.
...
## 13. Производительность и известные границы версии 0.3

- Цель — 30 fps; один Metal draw call с аналитическими сферами, локальные текстуры с mipmaps. CPU вычисляет только состояние и часы; местное время обновляется раз в секунду, геометрия — каждый кадр.
- До трёх GPU-команд в работе, кадр пропускается при заполнении очереди. Максимальная сторона Metal drawable — 2560 пикселей; часы остаются в разрешении AppKit/Retina.
...
- Облака и карты поверхности — исторические композиты NASA, **не текущая погода**. Млечный Путь и звёзды художественные, не астрономический каталог.
...
- Каждый экземпляр заставки имеет собственную композицию и Metal-ресурсы. Два экземпляра протестированы в хосте; реальные несколько мониторов ещё не проверялись. Расход памяти растёт с числом дисплеев.
- Не проведены длительное измерение энергопотребления на M1, Instruments-профилирование и проверка реального системного lock screen. 30 fps — настройка, а не измеренная гарантия на всех устройствах.
...
Материал настроен по предоставленному ночному снимку NASA: чёрный фон, синие тени, золотистые города и тонкая атмосфера. Камера по умолчанию остаётся над городом наблюдателя. Сравнение: `Docs/Observer-Singapore.png` и `Docs/Reference-day.png` — 5 сентября 2026, 23:00 и 12:00 UTC, Madrid. Исходные карты 8K, GPU-карты 4K. Атрибуция: `Docs/Assets.md`, `Resources/ThirdParty.txt`.
...
- `Docs/Observer-Madrid-overview.png`: тот же момент, явно обозначенное сжатое расстояние. Этот вариант сохраняет крупный вид Земли, но не является масштабной картой расстояния.

Все сохранённые кадры имеют подпись тестового времени. При обычном запуске preview использует текущие часы Mac. В меню «Вид → Настроить сцену» доступны ручной город и переключатель «Реальный масштаб расстояния Земля–Луна». В `.saver` нет диагностических подписей.
...
Последнее уточнение пользователя: по умолчанию используется единый масштаб размеров Земли, Луны и расстояния между ними. Вся сцена уменьшается, чтобы поместиться на экране. Сжатый обзор остаётся дополнительным режимом с явной подписью.
