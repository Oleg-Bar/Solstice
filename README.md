...
**Текущая версия: Solstice 1.08.** Проект находится на этапе подготовки к распространению. Перед публичной продажей готовую сборку необходимо подписать сертификатом Apple Developer ID и нотариализировать.

Рабочее название движка и приложения — **Terra**, название проекта и продукта — **Solstice**.

## Установка для покупателей

Готовый файл релиза называется `Solstice-1.08-macOS-Apple-Silicon.dmg` и предназначен для Mac с процессорами Apple Silicon (M1 и новее), macOS 13 или новее.

1. Откройте загруженный DMG.
2. Дважды нажмите **Install Solstice**.
3. При желании сначала выберите **Предпросмотр**, затем нажмите **Установить**.
4. В открывшихся настройках macOS выберите **Solstice** в списке заставок.

Установщик работает только в учётной записи текущего пользователя и копирует `Solstice.saver` в `~/Library/Screen Savers/`; права администратора для этой операции не нужны. Перед публичным распространением DMG, установщик и заставка должны быть подписаны сертификатом Apple Developer ID и нотариализированы.
...
## 13. Производительность и известные границы версии 1.08
